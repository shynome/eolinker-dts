...
const PaseResult = (result_rules, result_params = []) => {
  ...
  let fields = result_params
    .filter(r => typeof r.paramKey === 'string' && r.paramKey !== "")
    .map(v => {
      let extra = result_rules.filter(r => r.paramKey === v.paramKey)[0];
      return /**@type {import('./dealObjectType.type').CommonParams} */ ({
        ...v.toJSON(),
        paramType: extra.paramType,
        paramValue: extra.paramValue
      });
    });
